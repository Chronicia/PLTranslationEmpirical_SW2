...
python3 src/test/compile_avatar.py --source_lang $SRC_LANG --target_lang $TRG_LANG --model $MODEL --report_dir $OUTPUT_DIR --timestamp "$TIMESTAMP";
python3 src/test/compile_avatar_feedback.py --source_lang $SRC_LANG --target_lang $TRG_LANG --model $MODEL --report_dir $OUTPUT_DIR --attempt $ATTEMPT --timestamp "$TIMESTAMP";

...
class Translate:
    ...
    def translate_with_OPENAI(self, source, code_as_str, to):
        system_prompt = """Translate the provided Python code into equivalent Go code with a focus on accuracy and functionality. Ensure the Go code is idiomatic and adheres to best practices in Go programming. Minimize potential issues by addressing the following:
Syntax: Correctly map Python constructs to their Go equivalents.
Type system: Accurately handle Python's dynamic typing to Go's static typing.
Error handling: Replace Python exceptions with Go's error-handling patterns.
Data structures: Convert Python-specific structures (e.g., lists, dictionaries) to appropriate Go types (e.g., slices, maps).
Concurrency: Adapt Python's threading or async patterns to Go's goroutines and channels where applicable.
Libraries: Replace Python standard library and third-party modules with corresponding Go packages.
